...
└─ Face Attribute
  ...
  ├─ scripts
    ...
    ├─ run_standalone_train_.sh          # launch standalone training(1p) in 
    ├─ run_distribute_train_.sh          # launch distributed training(8p) in 
    ├─ run_eval.sh                          # launch evaluating in ascend
    ├─ run_eval_.sh                      # launch evaluating in 
...
- Stand alone mode
    ...
    ```bash 
    cd ./scripts
    sh run_standalone_train_.sh [MINDRECORD_FILE] [CUDA_VISIBLE_DEVICES]
    ```
    ...
    ```bash 
    cd ./scripts
    sh run_standalone_train_.sh [MINDRECORD_FILE] [CUDA_VISIBLE_DEVICES] [PRETRAINED_BACKBONE]
    ```
    ...
    ```bash
    cd ./scripts
    sh run_standalone_train_.sh /home/train.mindrecord 0 /home/a.ckpt
    ```
...
- Distribute mode (recommended)
    ...
    ```bash 
    cd ./scripts
    sh run_distribute_train_.sh [DEVICE_NUM] [CUDA_VISIBLE_DEVICES(0,1,2,3,4,5,6,7)] [MINDRECORD_FILE]
    ```
    ...
    ```bash 
    cd ./scripts
    sh run_distribute_train_.sh [DEVICE_NUM] [CUDA_VISIBLE_DEVICES(0,1,2,3,4,5,6,7)] [MINDRECORD_FILE] [PRETRAINED_BACKBONE]
    ```
    ...
    ```bash 
    cd ./scripts
    sh run_distribute_train_.sh 8 0,1,2,3,4,5,6,7 /home/train.mindrecord ./rank_table_8p.json /home/a.ckpt
    ```
...
```bash 
cd ./scripts
sh run_eval_.sh [MINDRECORD_FILE] [CUDA_VISIBLE_DEVICES] [PRETRAINED_BACKBONE]
```
